...
class Stats:
   ...
   def distribucio_lluminositat(self):
       """
       Return a dict {lluminositat: count} ordered by accident frequency.
       Fills missing values with 'Desconegut'.
       """
       if self.df.empty or "D_LLUMINOSITAT" not in self.df.columns:
           return {}
       counts = self.df["D_LLUMINOSITAT"].fillna("Desconegut").value_counts()
       return counts.to_dict()
   ...
   def distribucio_velocitats(self, top_n=6):
       ...
       """
       if self.df.empty or "C_VELOCITAT_VIA" not in self.df.columns:
           return {}
       counts = self.df["C_VELOCITAT_VIA"].dropna().value_counts().head(top_n)
